...
public class OffScreenUI_Cull : MonoBehaviour {
    ...
    [SerializeField] public GameObject[] _optionalGO_t_eyes_n_Off;
    ...
    void Update() {
        ...
        int prevLength = _optionalGO_t_eyes_n_Off.Length;

        _optionalGO_t_eyes_n_Off = _optionalGO_t_eyes_n_Off.Where(go => go != null)
                                                     .ToArray();

        if(_optionalGO_t_eyes_n_Off.Length != prevLength) {
    ...
    void toggleElements_ifNeeded(bool requiredValue) {
        ...
        for (int i = 0; i < _optionalGO_t_eyes_n_Off.Length; i++) {
            GameObject optionalGO = _optionalGO_t_eyes_n_Off[i];
